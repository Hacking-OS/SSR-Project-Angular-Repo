...
// @NgModule({})
@Injectable()
export class HttpMainEndpointService {
  ...
  //     catchError((error: HttpErrorResponse) : Observable<any> => {
  //       console.error(error);
  //       return throwError(() => new Error(error.message)) as Observable<HttpErrorResponse>;
  //       // Uncomment and implement the retry logic if needed
  //       // return this.handleError(error, () => this.GetApiResponse<Response, OtherParams>(apiPath, obj)); // Retry if error occurs
  //     })
  //   );
  //   // return this.http.post<Response>(url, obj).pipe();
  // }
